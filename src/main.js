...
// Update JSON input when dice selector changes
function updateJsonFromSelector() {
    const selectedOptions = Array.from(diceSelector.selectedOptions);
    const diceConfig = selectedOptions.map(option => ({
        dice: option.value
    }));
    jsonInput.value = JSON.stringify(diceConfig);
}

// Initialize
diceSelector.addEventListener('change', updateJsonFromSelector);
updateJsonFromSelector(); // Set initial value

function rollDice() {
    updateJsonFromSelector(); // Make sure JSON is in sync
    ...
    if (found === false) {
      ...
      diceRolls.forEach((diceRoll) => {
          ...
          for (let i = 0; i < repeatCount; i++) {
            ...
            const die = createDie(diceRoll.dice, false, i === 0, undefined, undefined, diceMaterial, scene, world, diceRoll.diceColor, diceRoll.textColor, diceRoll.backgroundColor, isSecret);
            if (!die) return;
      ...
      diceRolls.forEach((diceRoll) => {
          ...
          for (let i = 0; i < repeatCount; i++) {
            ...
            const die = createDie(diceRoll.dice, true, i === 0, diceRoll.rolled, closestIndexes[0], diceMaterial, scene, world, diceRoll.diceColor, diceRoll.textColor, diceRoll.backgroundColor, isSecret);
            if (!die) return;
