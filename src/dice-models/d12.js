...
import { D12_GEOMETRY, getChamferGeometry, makeGeometry } from '../geometry.js';
import { createFaceTexture } from '../face-texture.js';

export function createD12Mesh(size, targetNumber, foundClosestIndex, diceColor = 0xf0f0f0, textColor = '#FFFFFF', backgroundColor = '#f39c12', isSecret = false, decals = null, decalRegistry = null) {
    const radius = size * 0.9;
    const tab = 0.2;
    const af = -Math.PI / 4 / 2;

    const vectors = D12_GEOMETRY.vertices.map(v => new THREE.Vector3().fromArray(v).normalize());

    const chamferGeometry = getChamferGeometry(vectors, D12_GEOMETRY.faces, 0.968);
...
export function createD12Body(size, material) {
    const cannonVertices = D12_GEOMETRY.vertices.map(v => {
        const vec = new CANNON.Vec3(v[0], v[1], v[2]);
        return vec.unit().scale(size * 0.9);
    });
    const cannonFaces = D12_GEOMETRY.faces.map(face => face.slice(0, 5));
